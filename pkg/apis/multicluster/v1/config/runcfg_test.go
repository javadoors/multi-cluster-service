...
func TestNewRunConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			runtimePatch := gomonkey.ApplyFunc(runtime.NewServerConfig, func() *runtime.ServerConfig {
				return &runtime.ServerConfig{}
			})
			k8sPatch := gomonkey.ApplyFunc(k8s.NewKubernetesCfg, func() *k8s.KubernetesCfg {
				return &k8s.KubernetesCfg{}
			})
			defer runtimePatch.Reset()
